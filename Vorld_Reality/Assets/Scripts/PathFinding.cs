...
public class PathFinding : MonoBehaviour
{
    ...
    public Target TargetPosition;//Starting position to pathfind to
    public List<Node> Path;

    private void Awake()//When the program starts
    {
        //GridReference = GetComponent<GRID>();//Get a reference to the game manager
        GridReference = GetComponentInParent<GRID>();
    }

    private void Start()//Every frame
    {
        StartCoroutine(Wait());
        GridReference = GetComponentInParent<GRID>();
        FindPath(StartPosition.position, TargetPosition.target);//Find a path to the goal     
    }

    private void Update()
    {
        GridReference = GetComponentInParent<GRID>();
        FindPath(StartPosition.position, TargetPosition.target);
    }
